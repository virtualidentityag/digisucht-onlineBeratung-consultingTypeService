...
class AuthorisationServiceTest {
  ...
  void extractRealmRoles_Should_ExtractAuthoritiesFromJwt() {
    ...
    when(jwt.getClaims())
        .thenReturn(
            new HashMap<>(
                Map.of("realm_access", Map.of("roles", Lists.newArrayList("topic-admin")))));
    ...
    assertThat(grantedAuthorities.stream().map(GrantedAuthority::getAuthority))
        .contains(
            "AUTHORIZATION_CREATE_TOPIC",
            "AUTHORIZATION_UPDATE_TOPIC",
            "AUTHORIZATION_GET_TOPICS_TRANSLATION_BY_ID",
            "AUTHORIZATION_GET_ALL_TOPICS");
  }
}
